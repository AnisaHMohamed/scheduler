...
import { render, cleanup, waitForElement, fireEvent, queryByAltText, prettyDOM, getAllByTestId, getByAltText, getByPlaceholderText, queryByText, getByTestId} from "@testing-library/react";
...
describe("Application", () => {
  ...
  it("loads data, books an interview and reduces the spots remaining for Monday by 1", async () => {
    ...
    const dayAfterAdd = getAllByTestId(container, "day").find(day =>queryByText(day, "Monday"));
    const spotsRemainingAfterAdd = getByTestId(dayAfterAdd, "spotsRemaining").textContent;
  ...
  it("loads data, cancels an interview and increases the spots remaining for Monday by 1", async () => {
    ...
    const dayAfterDelete = getAllByTestId(container, "day").find(day =>queryByText(day, "Monday"));
    const spotRemainingAfterDelete = getByTestId(dayAfterDelete, "spotsRemaining").textContent;
  ...
  it("loads data, edits an interview and keeps the spots remaining for Monday the same", async () => {
    ...
    fireEvent.change(getByPlaceholderText(appointment, /enter student name/i), {
      target: { value: "Anisa Mohamed" }
    });
    fireEvent.click(getByAltText(appointment, "Sylvia Palmer"));

    fireEvent.click(getByText(appointment, "Save"));
    expect(getByText(appointment, "Saving")).toBeInTheDocument();
    await waitForElement(() => queryByText(appointment, "Anisa Mohamed"));

    const spotsRemainingAfterAdd = getByTestId(day, "spotsRemaining").textContent;
